...
function setStatus(text, close, context)
    ...
    elseif (close == 'tos') then
        ...
        refs.thirdButton.MouseButton1Click:Connect(function()
            setclipboard('yeah just agree thanks');
        ...
        refs.button.MouseButton1Click:Connect(function()
            ...
            if (not isfolder('Rukia Hub V1')) then
                makefolder('Rukia Hub V1');
            end;

            writefile('Rukia Hub V1/scripts/internal.json', HttpService:JSONEncode({
                agreed = true
            }));
    ...
    elseif (close == 'error') then
        ...
        task.delay((text == 'Script already ran' or text == 'All done' or text == 'Thank you!' or text == 'Script successfully loaded!') and 1 or 8, function()
            if (destroyUI()) then
                statusEvent:Destroy();
                getgenv().ah_statusEvent = nil;
            end;
...
local function hasAgreedToTOS()
    if (isfile('Rukia Hub V1/scripts/internal.json')) then
        local content = readfile('Rukia Hub V1/scripts/internal.json');
        local success, data = pcall(function()
            return HttpService:JSONDecode(content);
        end);

        if (success and data.agreed) then
            return true;
        end;
    end;
    return false;
...
local function requireTOS(callback)
    if (hasAgreedToTOS()) then
        callback();
        return;
    end;

   setStatus('Please agree to the Terms of Service!', 'tos');
    repeat 
        task.wait();
        hasAgreedToTOS();
    until hasAgreedToTOS();
...
xpcall(function()
    ...
    if (typeof(websiteKey) ~= 'string') then print('oh') return end;
    ...
    local suc, err = pcall(function()
        if (not isfolder('Rukia Hub V1')) then makefolder('Rukia Hub V1'); end;
        if (not isfolder('Rukia Hub V1/scripts')) then makefolder('Rukia Hub V1/scripts'); end;
    end);
    ...
    task.spawn(function()
        metadataRequest = httpRequest({
            Url = 'https://rukiascripts.xyz/metadata.json'
        });
    end);
    ...
    if (not fileName) then
        -- If no file name then we load the smallest file possible which in this case is Baseplate
        fileName = 'Baseplate';
    end;
