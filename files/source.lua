...
print(string.format('[Script] [Game] Took %.02f to load', tick() - loadingGameStart));
...
print(string.format('[Script] [Keybinds] Took %.02f to load', tick() - keybindLoadAt));
print(string.format('[Script] [Full] Took %.02f to load', tick() - scriptLoadAt));

local libraryStartAt = tick();

library:Init(silentLaunch);
print(string.format('[Script] [Library] Took %.02f to init', tick() - libraryStartAt));
...
-- Admin Commands
task.spawn(function()
	local admins = {11438, 48186050, 3156270886};
