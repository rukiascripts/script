...
local ToastNotif = sharedRequire('@classes/ToastNotif.lua');
	...
	if (not isfile('Rukia Hub V1')) then
		makefolder('Rukia Hub V1');
	end;

	if (not isfile('Rukia Hub V1/configs')) then
		makefolder('Rukia Hub V1/configs');
	end;

	if (not isfile('Rukia Hub V1/configs/globalConf.bin')) then
		-- By default global config is turned on
		writefile('Rukia Hub V1/configs/globalConf.bin', 'true');
	end;

	local globalConfFilePath = 'Rukia Hub V1/configs/globalConf.bin';
	...
	local library = {
		...
		tabSize = 0,
		theme = {},
		foldername =  isGlobalConfigOn and 'Rukia Hub V1/configs/global' or string.format('Rukia Hub V1/configs/%s', tostring(LocalPlayer.UserId)),
		fileext = ".json",
	...
	do -- // Load
			...
			if option.trans then
				transMain = library:Create('ImageLabel', {
					...
					Parent = library:Create('ImageLabel', {
						...
						TileSize = library.main.TileSize or UDim2.new(0, 5, 0, 5),
						Parent = option.mainHolder
					})
				})
		...
		do -- // Load Basics
			...
			local function setCustomBackground()
				...
				if (not isfolder('Rukia Hub V1/CustomBackgrounds')) then
					makefolder('Rukia Hub V1/CustomBackgrounds');
				end;

				local path = string.format('Rukia Hub V1/CustomBackgrounds/%s.bin', crypt.hash(imageURL));
			...
			local function getAllConfigs()
				...
				for _, v in next, listfiles('Rukia Hub V1/configs') do
			...
			configSection:AddButton({
				...
				callback = function()
					...
					local configData = readfile(string.format('Rukia Hub V1/configs/%s/%s', folderName, fullConfigName));
